...
async fn test_ensure_message_box_cache_hit_returns_same_id() {
    ...
    let from_db = get_message_box_id(&pool, TEST_KEY, box_name).await.unwrap();
...
async fn test_ensure_message_box_cache_distinct_rooms() {
    ...
    let id_other = ensure_message_box(&pool, TEST_KEY2, "room_a")
        .await
        .unwrap();
    assert_ne!(
        id_a, id_other,
        "same box name under a different key is a different room"
    );
}
...
async fn test_ensure_message_box_cache_invalidation_reresolves() {
    ...
    assert_eq!(
        id1, id2,
        "re-resolve after invalidation must match the durable id"
    );
}
